...
export default function RequestPageClient({
  ...
  const updateStatus = async (
    ...
    setIsLoading(true);
    if (status === "Fulfilled") {
      const { success, message } = await fulfillRequest({
        id: data.id,
      ...
      toast.success("Request Fulfilled! Redirecting...");
      await delay(3000);
      redirect("/", RedirectType.push); // this also returns the function
    }

    const { success } = await updateRequestStatusById({
      id: data.id,
      status,
    });

    if (success) {
      toast.success("Status Updated! Redirecting...");
      await delay(3000);
